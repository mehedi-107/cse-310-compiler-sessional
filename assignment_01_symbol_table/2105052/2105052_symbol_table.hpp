...
class ScopeTable{
    ...
    public:
        int collision_cnt;
        ScopeTable(int num_bucket,int table_id,uint (*hash_func)(string,uint), ScopeTable* parent_scope = nullptr) {
        ...
        SymbolInfo* look_up(string symbol){
            int hash_index = hash_func(symbol,this->num_bucket);
            SymbolInfo* curr_ptr = this->hashTable[hash_index];
            // dbg(hash_index);
        ...
        void print(int indent=0){
            ...
            cout<<spaces<<"ScopeTable# "<<this->table_id<<endl;
            for(int i =0; i<this->num_bucket; i++){
                SymbolInfo* curr_ptr = this->hashTable[i];
                cout<<spaces<<i+1<<"--> ";
                while(curr_ptr!=nullptr){
...
class SymbolTable{
    ...
    public:
        SymbolTable(int num_bucket,uint (*hash_func)(string,uint) = SDBMHash){
            ...
            this->num_bucket = num_bucket;
            this->table_id = 1;
            curr_scope = new ScopeTable(num_bucket,table_id++,hash_func);
            collision_cnt = 0;
        }

        void enter_scope(){
            ScopeTable* new_scope = new ScopeTable(num_bucket,table_id++,hash_func,curr_scope);
        ...
        SymbolInfo* look_up(string symbol){
            ScopeTable* temp = curr_scope;
            while(1){
                SymbolInfo* looked_up = temp->look_up(symbol);
